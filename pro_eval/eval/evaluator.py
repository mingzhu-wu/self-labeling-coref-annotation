...
def pro(clusters, mention_to_gold):
    tp, p = 0, 0
    for c in clusters:
        pronouns = []
        non_pronouns = []
        for m in c:
            if m.is_pronoun:
                ...
                non_pronouns.append(m)
        p += len(pronouns)
        for pr in pronouns:
            if pr in mention_to_gold:
                p_label = mention_to_gold[pr]
                for m in non_pronouns:
                    if m in mention_to_gold:
                        if mention_to_gold[m]== p_label:
                            tp +=1
                            break
